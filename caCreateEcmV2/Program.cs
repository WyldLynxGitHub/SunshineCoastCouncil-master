    ...
    class Program
    {
        ...
        private static void CreateNewrecords()
        {
            ...
            using (SqlConnection con = new SqlConnection("Data Source=Cl1025;Initial Catalog=StagingMove;Integrated Security=True"))
            {
                ...
                {
                    case "ep":
                        strsql = "Select * from [Extract Public] Where ID in(SELECT MIN(ID) AS ID FROM [Extract Public] GROUP BY [STD:DocumentID]) and [STD:Version] = 1 and [STD:DocumentSetID] in (Select DocumentSetID from ECM_MasterCheck Where OrigBatch = 'Extract Public' and rmuri is null) Order by [ID] OFFSET " + strOffset + " ROWS FETCH NEXT " + strFetch + " ROWS ONLY";
                        break;
                    case "epz":
                        strsql = "Select * from [Extract Public Zip] Where ID in(SELECT MIN(ID) AS ID FROM [Extract Public Zip] GROUP BY [STD:DocumentID]) and [STD:Version] = 1 and [STD:DocumentSetID] in (Select DocumentSetID from ECM_MasterCheck Where OrigBatch = 'Extract Public Zip' and rmuri is null) Order by [ID] OFFSET " + strOffset + " ROWS FETCH NEXT " + strFetch + " ROWS ONLY";
                        break;
                    case "elai":
                        strsql = "Select * from [Elai] Where ID in(SELECT MIN(ID) AS ID FROM [Elai] GROUP BY [STD:DocumentID]) and [STD:Version] = 1 and [STD:DocumentSetID] in (Select DocumentSetID from ECM_MasterCheck Where OrigBatch = 'Linked To Application' and rmuri is null) Order by [ID] OFFSET " + strOffset + " ROWS FETCH NEXT " + strFetch + " ROWS ONLY";
                        break;
                    case "elpi":
                        strsql = "Select * from [Elpi] Where ID in(SELECT MIN(ID) AS ID FROM [Elpi] GROUP BY [STD:DocumentID]) and [STD:Version] = 1 and [STD:DocumentSetID] in (Select DocumentSetID from ECM_MasterCheck Where OrigBatch = 'Linked To Property' and rmuri is null) Order by [ID] OFFSET " + strOffset + " ROWS FETCH NEXT " + strFetch + " ROWS ONLY";
                        break;
                }
                if(strsql==null)
                {
        ...
        private static void processtoEddie(List<ECMMigration> lstecm)
        {
            ...
            using (Database db = new Database())
            {
                ...
                foreach (ECMMigration ecm in lstecm)
                {
                    ...
                    Console.WriteLine("Processing {0} - Number {1} of {2}, time since start {3}", ecm.DocSetID, iCount.ToString(), strTotal,result.TotalMinutes.ToString());
                    try
                    {
